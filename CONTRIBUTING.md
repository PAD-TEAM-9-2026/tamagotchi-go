...
4. Update the README and `docs/field-types.md` together when the shared communication contract changes. Request review from affected service owners.
...
For deployment changes, also validate Compose with a local `deploy/.env` and verify the affected services run. For Postman changes, run the affected requests against the deployed services. The contract checker validates documentation links and payload names; it does not test service behavior.
